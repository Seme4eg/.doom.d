...
(after! gptel
  ...
  ;; DEPRECATED: google somehow blocks my proxy idk
  ...
  (defun sad/init-deepseek (fn &rest args) ;; &rest _args
    (unless (or (string= (gptel-backend-name gptel-backend) "DeepSeek")
      ...
      (setq gptel-backend (gptel-make-deepseek "DeepSeek"
                            ...
                            :stream t)))
    (apply fn args))
  ;; (advice-add 'gptel :around #'sad/init-deepseek)

  (defun sad/init-sambanova (fn &rest args) ;; &rest _args
    (unless (or (string= (gptel-backend-name gptel-backend) "Sambanova")
      ...
      (setq gptel-backend (gptel-make-openai "Sambanova"
                             ...
                             :models '(DeepSeek-R1))))
    (apply fn args))
  (advice-add 'gptel :around #'sad/init-sambanova)
