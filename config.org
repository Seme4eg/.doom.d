...
(add-to-list 'safe-local-variable-values '(sqlformat-command . sqlfluff))
(add-to-list 'safe-local-variable-values '(sqlformat-args . ("--config=/home/earthian/.config/sqlfluff/config.cfg")))

(setq
 completion-ignore-case t
 +zen-text-scale 0.5
 enable-recursive-minibuffers t ;; helps emacs not hang when vertico recursing
...
(with-eval-after-load 'ssh-deploy
  ...
  (add-to-list 'ssh-deploy-exclude-list "\.org$")
  (add-to-list 'ssh-deploy-exclude-list "\.gitattributes$")
  (add-to-list 'ssh-deploy-exclude-list "\\.claude/")
  (add-to-list 'ssh-deploy-exclude-list "\\.git-crypt/"))
...
(with-eval-after-load 'lsp-mode
  (set-formatter! 'yamlfmt '("yamlfmt" "-in") :modes '(yaml-mode yaml-ts-mode))
  (setq-hook! 'yaml-mode-hook +format-with '(yamlfmt)))
...
*** Hlissner [[https://github.com/hlissner/.doom.d][#source]]
#+begin_src emacs-lisp
(setq
...
(map! :map doom-leader-map :desc "Cheat-sh" "h h" 'cheat-sh-search-topic)
...
*** TODO Golang (setup golangci lint & fix)
#+begin_src emacs-lisp
...
(with-eval-after-load 'go-ts-mode
  ...
  (set-formatter! 'parsefix '("parsefix" "-f" filepath) :modes '(go-mode go-ts-mode))
  (set-formatter! 'golines '("golines" "-m" "80" filepath) :modes '(go-mode go-ts-mode))
  (set-formatter! 'goimports '("goimports" filepath) :modes '(go-mode go-ts-mode))
  (setq-hook! 'go-ts-mode-hook +format-with '(goimports golines))
  ...
  (map! :map go-ts-mode-map :localleader :n "z" 'dap-breakpoint-toggle))
...
(with-eval-after-load 'dash-docs (setq dash-docs-browser-func #'eww))
#+end_src
...
(map! :after telega :leader
      ...
      "/" #'telega-chatbuf-filter-search ;; _ key to cancel search
      "t" #'telega
...
(with-eval-after-load 'telega
  ...
  (telega-patrons-mode nil)
  (telega-active-stories-mode nil)
  (telega-active-locations-mode nil)
  (global-telega-squash-message-mode)
  ;; (auto-fill-mode)

  (require 'telega-dired-dwim)

  ;; https://zevlg.github.io/telega.el/#using-proxy-to-access-telegram
  (when (string-equal (getenv "AT_HOME") "true")
  ...
  (setq
   ...
   ;; debloat
   ;; view aux? linkpreviewoption voicerecognition
   telega-chat-buffers-limit 1
   telega-msg-temex-show-reactions '(return nil)
   telega-sticker-set-show-cover nil
   telega-vvnote-play-speeds '(2 1.5 2.5 1) ;; 1 being last deliberately
  ...
  ;; telega-chat-footer-insexp '(progn
  ;;                              (when telega-chatbuf--messages-compact-view (telega-ins "\n"))
  ;;                              ;; nope, thank you
  ...
  ;;                              (telega-chatbuf-footer-ins-modes) (telega-chatbuf-footer-ins-bot-description)
  ;;                              (telega-chatbuf-footer-ins-join-button)
  ;;                              (telega-chatbuf-footer-ins-aux-plist :with-avatar-p telega-chat-show-avatars)))
...
#+end_src
*** Slack
_Some rarely used commands under prefix_:
- TODO: slack-message-... - commands for message compose map which is currently borked
- slack-...-update - update whatever
- slack-dialog-buffer-...
  - slack-dialog-edit-...
- slack-channel-...
- slack-file-...
...
(defun my/slack ()
  (interactive)
  (unless (boundp 'slack-team)
    (slack-register-team
     :name "MUSE"
     :token (+pass-get-field "work/mg/slack" "token")
     :cookie (+pass-get-field "work/mg/slack" "cookie")
     :full-and-display-names t
     :visible-threads t
     :default t
     :subscribed-channels nil)
    (oset slack-current-team enterprise-token (+pass-get-field "work/mg/slack" "enterprise-token"))
    (slack-start))
  (slack-im-select))

(defun my/slack-notifier (msg room team)
  (let ((title (concat team " | " room)))
    (notifications-notify :title title :body msg)))

(map! :map doom-leader-open-map "s" 'my/slack)

(map! :after slack :leader
      :prefix ("v" . "slack")
      "S" #'slack-stop
      "c" #'slack-im-select
      "C" #'slack-im-open
      ...
      "t" #'slack-all-threads
      "f" #'slack-file-upload-quick
      "X" #'slack-kill-all-buffers)

(add-hook 'slack-message-buffer-mode-hook 'visual-line-mode)

;; (advice-add 'slack-im-select :before #'slack-im-list-update)
...
(with-eval-after-load 'slack
  ...
  (setq
   ...
   slack-log-time-format "[%d-%m %H:%M:%S]"
   slack-message-custom-notifier #'my/slack-notifier
   slack-typing-visibility 'never
   slack-prefer-current-team t
   slack-image-file-directory "/tmp/emacs-slack-images/"
   slack-file-dir "~/Downloads/slack"))
...
;; NOTE: don't use the 'evil-respect-visual-line-mode var as then deleting /
;; yanking lines behavior also repects visual line and is very inconvenient. The
;; only thing i actually need is the movement.
...
(with-eval-after-load 'org
  ...
  (add-to-list 'org-capture-templates '("c" "Country" entry
                                        (file "~/Documents/00-travel/readme.org")
                                        "* %?\n- _SIM_:\n- _Money_:\n  - ATM:\n- _Transportation_:\n- _Words_:\n- _Water_:\n- Food:"
                                        :immediate-finish t
                                        :empty-lines-after 1))
...
(set-popup-rules!
  '(("^\\*eww" :ignore t)
    ("^\\*slack*Edit*" :side bottom :height 0.16 :width 40 :quit t :select ignore :ttl 5)
...
#+begin_src elisp
;; (setq sambanova_key (+pass-get-field "ai/sambanova" "apikey")
;;       gemini_key (+pass-get-secret "ai/gemini")
;;       deepseek_key (+pass-get-field "ai/deepseek" "apikey"))
...
(with-eval-after-load 'gptel
  (gptel-make-openai "Sambanova"
    ...
    :stream t
    :key (lambda () (+pass-get-field "ai/sambanova" "apikey"))
    :models '(DeepSeek-R1))
  (gptel-make-gemini "Gemini"
    :key (lambda () (+pass-get-secret "ai/gemini"))
    :stream t)
  (gptel-make-deepseek "DeepSeek"
    :key (lambda () (+pass-get-field "ai/deepseek" "apikey"))
    :stream t)
  (setq gptel-default-mode 'org-mode
        ...
        ;; gptel-api-key "1234"
        gptel-api-key (lambda () (+pass-get-field "ai/deepseek" "apikey"))
        gptel-model 'deepseek-chat ;; 'deepseek-reasoner 'gpt-4-turbo
        gptel-display-buffer-action '(nil (body-function . select-window)))
  (when (string-equal (getenv "AT_HOME") "true")
    ;; v2raya-lite user service
    (setq gptel-proxy "localhost:20171")))
#+end_src
...
** Org-vcard
#+begin_src emacs-lisp
(with-eval-after-load 'org-vcard
...
** Helpful var update funcs & binds
#+begin_src emacs-lisp
;;; --- Edit a variable's value from a Helpful buffer -------------------

(defvar-local my/edit-var--symbol nil
  "Symbol whose value is being edited in this buffer.")

(define-minor-mode my/edit-var-mode
  "Minor mode for editing a variable's value then applying it."
  :lighter " EditVar"
  :keymap (let ((map (make-sparse-keymap)))
            (define-key map (kbd "C-c C-c") #'my/edit-var-commit)
            (define-key map (kbd "C-c C-k") #'my/edit-var-abort)
            map))

(defun my/edit-var-commit ()
  "Read the buffer as a Lisp value and assign it to the variable."
  (interactive)
  (let* ((sym my/edit-var--symbol)
         (val (condition-case err
                  (read (buffer-substring-no-properties (point-min) (point-max)))
                (error (user-error "Can't read value: %s"
                                   (error-message-string err))))))
    ;; Respect :set handlers on defcustoms; plain set-default otherwise.
    (if (custom-variable-p sym)
        (customize-set-variable sym val)
      (set-default sym val))
    (let ((b (current-buffer)))
      (quit-window) (kill-buffer b))
    (helpful-update)
    (message "Set %s to %S" sym val)))

(defun my/edit-var-abort ()
  "Cancel editing without touching the variable."
  (interactive)
  (let ((b (current-buffer)))
    (quit-window) (kill-buffer b))
  (message "Aborted"))

(defun my/helpful-edit-variable ()
  "Edit, in an elisp scratch buffer, the variable shown in this Helpful buffer.
The value (array, string, int, list, anything readable) is pretty-printed;
press \\[my/edit-var-commit] to apply or \\[my/edit-var-abort] to cancel."
  (interactive)
  (unless (derived-mode-p 'helpful-mode)
    (user-error "Not in a Helpful buffer"))
  (when (bound-and-true-p helpful--callable-p)
    (user-error "%s is a function, not a variable" helpful--sym))
  (let ((sym helpful--sym))
    (unless (boundp sym)
      (user-error "%s is not bound" sym))
    (let ((buf (get-buffer-create (format "*edit: %s*" sym)))
      ...
      (with-current-buffer buf
        ...
        (goto-char (point-min))
        (setq my/edit-var--symbol sym)
        (my/edit-var-mode 1)
        (setq-local header-line-format
                    (format " Editing %s — C-c C-c apply · C-c C-k cancel" sym)))
      (pop-to-buffer buf))))

(map! :map helpful-mode-map
      :leader
      :desc "Edit value" "e" #'my/helpful-edit-variable)
#+end_src
