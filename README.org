...
** Info
~1..9~ (Info-nth-menu-item) - Go to the node of the Nth menu item.           
~C-k/j~ (Info-backward/forward-node) - Go backward/forward one node, considering all nodes ...
~a~ (info-apropos) - Grovel indices of all known Info files on yo...
~I~ (Info-virtual-index) - Show a node with all lines in the index cont...
~i~ (Info-index) - Look up a string TOPIC in the index for this...
~S~ (Info-search-case-sensitively) - Search for a regexp case-sensitively.          
~s~ (Info-search) - Search for REGEXP, starting from point, and ...
~u~ (Info-up) - Go to the superior node of this node.          
~d~ (Info-directory) - Go to the Info directory node.                 
~C-] / RET~ (Info-follow-nearest-node) - Follow a node reference near point.
~C-o/t~ (Info-history-back) - Go back in the history to the last node visi...
~TAB~ (Info-history-forward) - Go forward in the history of visited nodes.
~<S-tab> / ~<tab>~ (Info-prev/next-reference) - Move cursor to the previous/next cross-reference ...

~SPC h i~ (info) - Enter Info, the documentation browser.         
~SPC h S~ (info-lookup-symbol) - Display the definition of SYMBOL, as found i...
~SPC h K~ (Info-goto-emacs-key-command-node) - Go to the node in the Emacs manual which des...
~SPC h RET~ (info-emacs-manual) - Display the Emacs manual in Info mode.         
~SPC h 4 i~ (info-other-window) - Like ‘info’ but show the Info buffer in anot...

~M-h~ (Info-help) - Enter the Info tutorial.                       

_'g' prefix:_
- ~g k/j~ (Info-prev/next) - Go to the "previous/next" node, staying on the sa...
- ~g ?~ (Info-summary) - Display a brief summary of all Info commands.
- ~g T~ (Info-toc) - Go to a node with table of contents of the c...
- ~g t~ (Info-top-node) - Go to the Top node of this file.
- ~g m~ (Info-menu) - Go to the node pointed to by the menu item n...
- ~g G~ (Info-goto-node) - Go to Info node named NODENAME.  Give just N...
- ~g 1..9~ (Info-nth-menu-item) - Go to the node of the Nth menu item.
- ~g L~ (Info-history) - Go to a node with a menu of visited nodes.
- ~g [ / ](TAB)~ (Info-prev/next-reference) - Move cursor to the previous/next
  cross-reference ...
