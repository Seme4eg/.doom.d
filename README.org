...
- [ ] (mu4e) What mu4e does not do -> smtpmail link -- learn how to send msgs
...
**** Header mode keybindings
+ Movement
  - ~j/k~   :: Moves to the next/previous header line
  - ]]/[[ :: Moves to next/previous unread message
  - ~J~     :: Jump to another mail directory
+ Toggles
  - ~zt~ :: Toggles threaded message display
  - ~zr~ :: Toggles related message display (toggle showing messages that r not from
    current search query)
+ Marking
  - ~d~ :: Marks message for deletion
  - ~m~ :: Marks message for move to folder
  - ~+~ :: Marks message for flagging
  - ~-~ :: Marks message for unflagging
  - ~%~ :: Marks based on a regex pattern
  - ~u~ :: Removes mark for message
  - ~U~ :: Unmarks all marks in the view
...
Many of the same keybindings work! Marking keys work on the currently viewed
message.
...
**** Search queries [[https://www.djcbsoftware.nl/code/mu/mu4e/Queries.html][#docs]]
...
You can also use logic statements like =and= , =not=: ="maildir:/Inbox" and from:eli
and docs=, =org and :from {username}=

~M-arrow-right/left~ - move forward/backward in search history

_Bookmarking queries_ - =M-x mu4e-headers-search-bookmark=
...
***** Composing Email [[https://www.djcbsoftware.nl/code/mu/mu4e/Writing-messages.html][#docs]]
...
_Ways to get to compose screen:_
- =C= while in the mail list
- =R= while selecting a mail in the mail list or while viewing an e-mail to compose a reply
- =F= while selecting a mail in the mail list or while viewing an e-mail to compose a forwarded mail
- =M-x mu4e-compose-new= or bind that command to a key!

-----

- ~C-c C-a~ :: add an attachment to e-mail (Drag and drop also works!)
- ~C-c C-d~ :: save a message as a draft. To get back to editing this message
  select the message in the headers list or open the email and press ~c e~.

To _sign email_ =mml-secure-message-sign-pgpmime= (while composing a mail) to mark it to be
signed once you send it.
...
[[https://yamadharma.github.io/ru/post/2021/01/22/mail-synchronization-mbsync/#синхронизация][mbsync configs for different email clients]], [[https://www.youtube.com/watch?v=olXpfaSnf0o][system crafters 2]], [[https://help.mail.ru/mail/mailer/popsmtp][mail ru official]]

/Also don't forget about [[https://github.com/iqbalansari/mu4e-alert][mu4e-alert]] and org-msg (~s o~ that)/
