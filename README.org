...
- ~S~ =embark-collect-snapshot= - produces a buffer listing all the current
  candidates.
- ~E~ =embark-eSwitchingxport= - command tries to open a buffer in an appropriate major mode
...
- & / go / <S-return> :: =eww-browse-with-external-browser=. In case of trouble
  viewing a website with EWW
- H/L :: *back / forward* url
